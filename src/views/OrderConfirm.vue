    ...
    <div class="container">
      <div class="checkout-order">
        ...
        <div class="check-step">
          <ul>
            <li class="cur"><span>配送</span>订单</li>
            <li class="cur"><span>提交</span>订单</li>
              ...
              <li v-for="item in cartList" v-if="item.checked=='1'">
                <div class="cart-tab-1">
                  <div class="cart-item-pic">
                    <img v-bind:src="item.icon" v-bind:alt="item.name">
                  </div>
                ...
                <div class="cart-tab-4">
                  <div class="item-price-total">{{(item.quantity * item.price) | currency('￥')}}</div>
                </div>
              </li>
            </ul>
          </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      init() {
        this.$axios.get("/buyer/cart/list").then((response) => {
          ...
          this.orderTotal = this.subTotal + this.shopping - this.discount;
        })
      },
      payMent() {
        this.$axios.post("/buyer/order/new", {
          addressId: this.$route.query.addressId,
          cartInfos: this.cartList
        }).then((response) => {
          let res = response.data;
          if (res.code == "0") {
            this.$router.push({
              path: 'orderSuccess?orderId=' + res.data.orderId + '&orderAmount=' + res.data.orderAmount
            })
          } else {
