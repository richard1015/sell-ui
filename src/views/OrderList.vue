  ...
  export default {
    ...
    computed: {
      ...
      totalPrice() {
        var money = 0;
        this.cartList.forEach((item) => {
          if (item.checked == '1') money += parseFloat(item.price) * parseInt(item.quantity);
        })
        return money;
      }
    },
    methods: {
      ...
      editCart(flag, item) {
        ...
        this.$axios.get("/buyer/cart/edit", {params: param}).then((result) => {
          let res = result.data;
          if (res.code == "0") {
            let num = 0;
            if (flag == 'add') {
      ...
      delCart() {
        ...
        this.$axios.get("/buyer/cart/del", {params: param}).then((result) => {
          let res = result.data;
          if (res.code == "0") {
            this.modalConfirm = false;
            this.init();
      ...
      toggleCheckAll() {
        ...
        this.$axios.get("/buyer/cart/edit_check_all", {params: param}).then((result) => {
          let res = result.data;
          if (res.code == "0") {
